...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "dataPath", help='Folder which include radarTrack.npy, output-keypoints-ByteTrack.npy and config', nargs=1, type=str)
    parser.add_argument("-s", "--start_frame",
                        help='Start frame which need to be processed', default=0)
    parser.add_argument(
        "-e", "--end_frame", help='End frame which need to be processed', default=4600)
    parser.add_argument(
        "--minv", help='Minimum row index of the image which is considered', default=0)
    parser.add_argument(
        "--maxv", help='Maximum row index of the image which is considered', default=700)
    parser.add_argument("--tao", help='CATS parameter tao', default=3)
    parser.add_argument("--eps", help='CATS parameter eps', default=20)
    parser.add_argument(
        "--visualize", help='Show ground plane points', action='store_true')
    ...
    if not os.path.exists('{}/radarTrack.npy'.format(DATA_PATH)):
        ...
        exit(0)
    if not os.path.exists('{}/output-keypoints-ByteTrack.npy'.format(DATA_PATH)):
        print('{}/output-keypoints-ByteTrack.npy not exist!'.format(DATA_PATH))
        exit(0)
    if not os.path.exists('{}/config'.format(DATA_PATH)):
        print('{}/config not exist!'.format(DATA_PATH))
        exit(0)
    radarTrack = np.load(
        '{}/radarTrack.npy'.format(DATA_PATH), allow_pickle=True)
    imageTrack = np.load(
        '{}/output-keypoints-ByteTrack.npy'.format(DATA_PATH), allow_pickle=True)
    ...
    for i in range(start, end):
        for j, item in enumerate(imageTrack[i]['ids']):
            pos = CalBottomMiddle(imageTrack[i]['keypoints'][j])
            if pos is None:
                continue
            now = {'frame': i, 'pos': pos}

            imageTrackID[item] = np.array([now]) if not item in imageTrackID.keys(
            ) else np.append(imageTrackID[item], now)
        ...
        for j, item in enumerate(radarTrack[i]['data']):
            ...
            now = {'frame': i, 'pos': np.array([x, y, z]), 'time': radarTrack[i]['time'], 'v': np.array([
                item['vx'], item['vy'], item['vz']])}

            radarTrackID[ID] = np.array([now]) if not ID in radarTrackID.keys(
            ) else np.append(radarTrackID[ID], now)
    ...
    ###################### CATS ##################################
    print('Bidirectional selection associating. It may take some minutes.')
    img2radar = dict()
    radar2img = dict()
    import time
    for idImg in imageTrackID.keys():
        now = [CATS(imageTrackID[idImg], radarTrackID[idRadar], tao, eps)
    ...
    for idRadar in radarTrackID.keys():
        now = [CATS(radarTrackID[idRadar], imageTrackID[idImg], tao, eps)
               for idImg in imageTrackID.keys()]
        ...
        while idx1 < len(nowImg) and idx2 < len(nowRadar):
            if nowImg[idx1]['frame'] == nowRadar[idx2]['frame']:
                ...
                if lastTime != -1:
                    ...
                    tk = TrackEKF(MeasurePoint(x=xR, y=yR, z=zR,
                                  vx=vxc, vy=vyc, vz=vzc), s2p)
    ...
    result = np.vstack((us, vs, xs, ys, zs)).T
    save_path = '{}/output-PlanePoint.npy'.format(OUTPUT_DIR)
    np.save(save_path, result, allow_pickle=True)
    print("Save ground plane point into {}".format(save_path))
