...
anno_path = '/mnt/datasets/radar_camera_bridge/2/annotation.npy'
...
for frame_idx, (now, anno) in tqdm(enumerate(zip(data[:frame_num], annotation[:frame_num])), total=frame_num):
    ...
    # Draw annotation results
    gt_pos = anno['pos_bottom_mid']
    gt_theta = anno['rvecs']
    gt_R = rotation_vectors_to_matrices(gt_theta)
    gt_ext = anno['sizes']

    gt_corner_point = corner_to_extend[np.newaxis, :, :] * gt_ext[:, np.newaxis, :]
    gt_corner_sensor = gt_pos[:, np.newaxis, :] + gt_corner_point @ gt_R.transpose(0, 2, 1)
    gt_corner_world = sensor_to_world_T[np.newaxis, np.newaxis, :] + (sensor_to_world_R @ gt_corner_sensor.transpose(0, 2, 1)).transpose(0, 2, 1)

    gt_corner_world_to_pix = (gt_corner_world[:, :4, :2] * 10).astype(int)
    gt_corner_world_to_pix[:, :, 0] += int(frame_width / 2)
    gt_corner_world_to_pix[:, :, 1] = frame_height - gt_corner_world_to_pix[:, :, 1]

    for j, obj in enumerate(gt_corner_world_to_pix):
        mid = np.max(obj, axis=0).astype(int)

        for i in range(3):
            try:
                cv2.line(frame, obj[i], obj[(i+1)%4], color=(255, 0, 0), thickness=5)
            except:
                pass
