...
class Space2Plane:
    ...
    def __init__(self, config=0):
        if type(config) == str:
            ...
            conf.read(config)
            self.cameraMatrix = np.array(json.loads(
                conf.get('cameraIntrinsic', 'cameramatrix')))
            self.distCoeffs = np.array(json.loads(
                conf.get('cameraIntrinsic', 'distcoeffs')))
            ...
            if config == 0:
                self.cameraMatrix = np.array([[1.90753223e+03, 0.00000000e+00, 6.44360570e+02],
                                              [0.00000000e+00, 1.98064319e+03,
                                                  3.65685545e+02],
                                              [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]], np.float)
                self.distCoeffs = np.array(
                    [0.0812306431723410, -0.100591860946000, 0, 0], np.float)
                self.gamma, self.theta, self.fa = (
                    506-500)/1000*2*pi, (499-500)/1000*2*pi, 0
                ...
                self.distCoeffs = np.array(
                    [0.0812306431723410, -0.100591860946000, 0, 0], np.float)
                self.gamma, self.theta, self.fa = (
                    266-500)/1000*2*pi, pi/2, (495-500)/1000*2*pi
                self.dx, self.dy, self.dz, self.k = 0, 0, 0, 1

            m1 = np.array([[cos(self.gamma), 0, sin(self.gamma)], [
                          0, 1, 0], [-sin(self.gamma), 0, cos(self.gamma)]])
            m2 = np.array([[1, 0, 0], [0, cos(
                self.theta), -sin(self.theta)], [0, sin(self.theta), cos(self.theta)]])
            m3 = np.array([[cos(self.fa), -sin(self.fa), 0],
                          [sin(self.fa), cos(self.fa), 0], [0, 0, 1]])
    ...
    def ColorFrame(self, frame, row, col):
        ...
        frame[row-self.sideLen:row+self.sideLen, col -
              self.sideLen:col+self.sideLen, :] = self.color
    ...
    def DrawMeasurementPoint(self, frame, p):
        ...
        res = cv2.projectPoints(
            p, self.R, self.T, self.cameraMatrix, self.distCoeffs)[0]
        for item in res:
    ...
    def xcyczc2uv(self, data):
        ...
        for item in data:
            zc = item[2]
            ret = np.append(
                ret, self.cameraMatrix[0, 2]+self.cameraMatrix[0, 0]*item[0]/zc)
            ret = np.append(
                ret, self.cameraMatrix[1, 2]+self.cameraMatrix[1, 1]*item[1]/zc)
    ...
    def uv2xcyc(self, data, xc1, yc1, zc1, u1, v1):
        ...
        for item in data:
            ...
            ret = np.append(
                ret, zc1/self.cameraMatrix[0, 0]*(item[0]-self.cameraMatrix[0, 2]))
            ret = np.append(
                ret, zc1/self.cameraMatrix[1, 1]*(item[1]-self.cameraMatrix[1, 2]))
        ret = np.reshape(ret, [-1, 2])
        return ret
    ...
    def uv2xczc(self, data, xc1, yc1, zc1, u1, v1):
        ...
        for item in data:
            ...
            ret = np.append(
                ret, (item[0]-self.cameraMatrix[0, 2])/self.cameraMatrix[0, 0]*zc2)
            ret = np.append(ret, zc2)
        ret = np.reshape(ret, [-1, 2])
        return ret
